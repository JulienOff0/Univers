--[[
   ⚡ Premium Hub v2 — Speed • Fly • Light • ESP • TP • NoClip • Infinite Jump
   Touche hide/show: "$" (Shift+4) • F2 • Insert • RightCtrl
   - Menu draggable, borné à l’écran, fluide
   - Contenu scrollable si nécessaire
   - Sliders: Speed, Jump Boost
   - ESP via Highlight + Billboard (zero Drawing API)
]]
...
local S = {
	...
	-- Speed
	speedOn = false,
	speed = 22, minSpeed = 16, maxSpeed = 120,
...
UI.Name = "PremiumHubV2"
safeParent(UI)

local Main = Instance.new("Frame")
Main.Size = UDim2.fromOffset(510, 540)
...
Shadow.Image = "rbxassetid://5028857084"; Shadow.ImageTransparency=.4
...
Shadow.Size = UDim2.new(1,32,1,32); Shadow.BackgroundTransparency=1
Shadow.Parent = Main
...
Title.Text = "⚡ Premium Hub v2 — Speed • Fly • Light • ESP • TP • NoClip • IJ"
Title.Parent = Header

local Hint = Instance.new("TextLabel")
Hint.BackgroundTransparency = 1
Hint.AnchorPoint = Vector2.new(1,0)
Hint.Position = UDim2.new(1,-42,0,10) -- place pour le bouton
Hint.Size = UDim2.fromOffset(280,24)
Hint.Font = Enum.Font.Gotham
Hint.TextSize = 14
Hint.TextXAlignment = Enum.TextXAlignment.Right
Hint.TextColor3 = Color3.fromRGB(180,180,190)
Hint.Text = 'Hide: "$"(⇧+4) • F2 • Insert • RightCtrl'
Hint.Parent = Header
...
Instance.new("UIStroke", HideBtn).Transparency = .35

local Sep = Instance.new("Frame")
Sep.BackgroundColor3 = Color3.fromRGB(45,45,52)
Sep.Size = UDim2.new(1,-24,0,1)
Sep.Position = UDim2.fromOffset(12,46)
...
local function refreshCanvas()
	Content.CanvasSize = UDim2.new(0,0,0,VList.AbsoluteContentSize.Y + 8)
end
...
local function makeCard(titleText, subtitleText, height)
	...
	T.BackgroundTransparency = 1; T.Font = Enum.Font.GothamSemibold; T.TextSize=16
	T.TextXAlignment=Enum.TextXAlignment.Left; T.TextColor3=Color3.fromRGB(235,235,240)
	T.Text = titleText; T.Position = UDim2.fromOffset(12,8); T.Size = UDim2.fromOffset(360,20); T.Parent = Card

	local ST = Instance.new("TextLabel")
	ST.BackgroundTransparency=1; ST.Font=Enum.Font.Gotham; ST.TextSize=13
	ST.TextXAlignment=Enum.TextXAlignment.Left; ST.TextColor3=Color3.fromRGB(170,170,182)
	ST.Text = subtitleText or ""; ST.Position = UDim2.fromOffset(12,30); ST.Size = UDim2.fromOffset(470,18); ST.Parent = Card
	return Card
end
local function makeToggle(parent, defaultOn, xOffset)
	...
	Btn.Position = UDim2.new(1, -(xOffset or 10), 0, 8)
...
local function makeSlider(parent, title, minV, maxV, defaultV, yOffset, onChanged)
	...
	Wrap.Position = UDim2.fromOffset(12, yOffset or 56)
	Wrap.Size = UDim2.new(1,-24,0,60)
	Wrap.Parent = parent
	local T = Instance.new("TextLabel")
	T.BackgroundTransparency=1; T.Font=Enum.Font.Gotham; T.TextSize=14
	T.TextXAlignment=Enum.TextXAlignment.Left; T.TextColor3=Color3.fromRGB(200,200,210)
	T.Text = string.format("%s: %d", title, defaultV)
	T.Position = UDim2.fromOffset(0,0); T.Size = UDim2.fromOffset(260,18); T.Parent = Wrap
	local Bar = Instance.new("Frame")
	Bar.BackgroundColor3 = Color3.fromRGB(45,45,55); Bar.BorderSizePixel=0
	Bar.Position = UDim2.fromOffset(0,26); Bar.Size = UDim2.new(1,0,0,8); Bar.Parent = Wrap
	Instance.new("UICorner", Bar).CornerRadius = UDim.new(0,6)
	local Fill = Instance.new("Frame")
	Fill.BackgroundColor3 = Color3.fromRGB(110,170,255); Fill.BorderSizePixel=0
	Fill.Size = UDim2.fromOffset(0,8); Fill.Parent = Bar
	Instance.new("UICorner", Fill).CornerRadius = UDim.new(0,6)
	local Knob = Instance.new("Frame")
	Knob.Size = UDim2.fromOffset(14,14); Knob.Position = UDim2.fromOffset(-7,-3)
	Knob.BackgroundColor3 = Color3.fromRGB(240,240,245); Knob.Parent = Fill
	Instance.new("UICorner", Knob).CornerRadius = UDim.new(1,0)
...
local function fly_disable()
	...
	if S.flyData then
		...
		-- Restore collisions we touched
		if S.flyData.collMap then
			for part,can in pairs(S.flyData.collMap) do
...
local function fly_enable()
	...
	lv.Attachment0 = att; lv.MaxForce = math.huge; lv.RelativeTo = Enum.ActuatorRelativeTo.World
	lv.Parent = hrp

	S.flyData = {hrp=hrp, humanoid=humanoid, lv=lv, att=att, Controls=Controls, collMap=collMap}

	local ENFORCE_DT = 0.05
	table.insert(S.flyConns, humanoid:GetPropertyChangedSignal("PlatformStand"):Connect(function()
		if S.flyAlive and humanoid.PlatformStand ~= true then humanoid.PlatformStand = true end
	end))
	table.insert(S.flyConns, RS.Stepped:Connect(function()
		if S.flyAlive then hrp.AssemblyAngularVelocity = Vector3.zero end
	end))
	task.spawn(function()
		while S.flyAlive and char.Parent do
			...
			task.wait(ENFORCE_DT)
		end
	end)

	-- main loop
	table.insert(S.flyConns, RS.RenderStepped:Connect(function(dt)
		if not S.flyAlive then return end
		local cam = workspace.CurrentCamera
		if not cam then return end
		local look = cam.CFrame.LookVector
		local yawDir = Vector3.new(look.X,0,look.Z)
		if yawDir.Magnitude < 1e-3 then yawDir = Vector3.zAxis else yawDir=yawDir.Unit end
		...
		local fwd, right = 0,0
		if UIS:IsKeyDown(Enum.KeyCode.W) then fwd += 1 end -- (Z en AZERTY)
		if UIS:IsKeyDown(Enum.KeyCode.S) then fwd -= 1 end
		if UIS:IsKeyDown(Enum.KeyCode.D) then right += 1 end
		if UIS:IsKeyDown(Enum.KeyCode.A) then right -= 1 end -- (Q en AZERTY)

		local SPEED, VERTICAL, PITCH_DEADZONE = 65, 45, 0.08
		local horiz = yawDir*fwd + rightDir*right
		if horiz.Magnitude>0 then horiz=horiz.Unit*SPEED else horiz=Vector3.zero end
		local vertical = 0
		if fwd ~= 0 then
			local pitch = look.Y
			if math.abs(pitch) > PITCH_DEADZONE then vertical = pitch*VERTICAL*fwd end
		end
...
local ROLE_COLORS = {
	...
	Sheriff = Color3.fromRGB(255,220,0),
	Detective = Color3.fromRGB(255,220,0),
}
...
local function ensureESPForChar(p, character)
	if not character or not p or p == LP then return end
	if not character:FindFirstChild("ESP_Highlight") then
		local h = Instance.new("Highlight")
		h.Name="ESP_Highlight"; h.FillTransparency=1; h.OutlineTransparency=0
		h.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop; h.Parent = character
	end
	local head = character:FindFirstChild("Head") or character:FindFirstChildWhichIsA("BasePart")
	if head and not head:FindFirstChild("ESP_BBG") then
		local bb = Instance.new("BillboardGui")
		bb.Name="ESP_BBG"; bb.Size=UDim2.new(0,210,0,48)
		bb.StudsOffset = Vector3.new(0,3,0); bb.AlwaysOnTop=true; bb.MaxDistance=5000; bb.Parent=head
		local nameLbl = Instance.new("TextLabel")
		nameLbl.Name="Name"; nameLbl.BackgroundTransparency=1; nameLbl.Size=UDim2.new(1,0,0.5,0)
		nameLbl.Font=Enum.Font.GothamBold; nameLbl.TextScaled=true; nameLbl.TextStrokeTransparency=.5; nameLbl.Parent=bb
		local distLbl = Instance.new("TextLabel")
		distLbl.Name="Dist"; distLbl.BackgroundTransparency=1; distLbl.Position=UDim2.new(0,0,0.5,0)
		distLbl.Size=UDim2.new(1,0,0.5,0); distLbl.Font=Enum.Font.Gotham; distLbl.TextScaled=true; distLbl.TextStrokeTransparency=.5; distLbl.Parent=bb
	end
end
		...
		if p ~= LP then
			...
			if char then
				...
				if tHRP and head then
					...
					local bb = head:FindFirstChild("ESP_BBG")
					local hl = char:FindFirstChild("ESP_Highlight")
					local tooFar = dist > S.espMaxDist
					if bb then bb.Enabled = not tooFar end
					if hl then hl.Enabled = not tooFar end
					if not tooFar then
						local c = colorFor(p, char)
						if hl then hl.OutlineColor = c end
						if bb then
							local nameLbl = bb:FindFirstChild("Name")
							local distLbl = bb:FindFirstChild("Dist")
							if nameLbl then
								if S.espNameWithUser then
									nameLbl.Text = string.format("%s (@%s)", p.DisplayName, p.Name)
								else nameLbl.Text = p.DisplayName end
								nameLbl.TextColor3 = c
							end
							if distLbl then
								distLbl.Text = string.format("%.0f studs", dist)
								distLbl.TextColor3 = c
							end
						end
					end
				end
...
local function esp_enable()
	...
	for _,p in ipairs(Players:GetPlayers()) do
		p.CharacterAdded:Connect(function(char) task.wait(.5); ensureESPForChar(p,char) end)
		if p.Character then ensureESPForChar(p, p.Character) end
	end
end
local function esp_disable()
	disconnect(S.espCon); S.espCon=nil
	-- Hide existing markers (do not destroy to reduce GC churn)
	for _,p in ipairs(Players:GetPlayers()) do
		local char = p.Character
		if char then
			local hl = char:FindFirstChild("ESP_Highlight"); if hl then hl.Enabled=false end
			local head = char:FindFirstChild("Head") or char:FindFirstChildWhichIsA("BasePart")
			if head then local bb=head:FindFirstChild("ESP_BBG"); if bb then bb.Enabled=false end end
		end
	end
end

-- ========== NoClip ==========
local function noclip_cachePart(inst)
	if inst:IsA("BasePart") and S.noclipOriginal[inst]==nil then
		S.noclipOriginal[inst] = inst.CanCollide
	end
end
local function noclip_cacheChar(char)
	for _,d in ipairs(char:GetDescendants()) do noclip_cachePart(d) end
	disconnect(S.noclipDesc)
	S.noclipDesc = char.DescendantAdded:Connect(noclip_cachePart)
end
local function noclip_apply(char)
	for part,_ in pairs(S.noclipOriginal) do
		if part.Parent and part:IsDescendantOf(char) then part.CanCollide=false end
	end
end
local function noclip_enable()
	local char = getChar()
	S.noclipOriginal = {}
	noclip_cacheChar(char)
	disconnect(S.noclipStep)
	S.noclipStep = RS.Stepped:Connect(function() noclip_apply(char) end)
end
...
local function tpToPlayerId(userId)
	...
	local myHRP = getHRPFromChar(me.Character)
	local targetHRP = getHRPFromChar(target.Character)
	if not (myHRP and targetHRP) then return end
	local cf = targetHRP.CFrame * CFrame.new(0,0,3)
	me.Character:PivotTo(cf)
end
local function listOthers()
	local arr = {}
	for _,p in ipairs(Players:GetPlayers()) do if p ~= LP then table.insert(arr,p) end end
	table.sort(arr, function(a,b)
		local ad=(a.DisplayName~="" and a.DisplayName) or a.Name
		local bd=(b.DisplayName~="" and b.DisplayName) or b.Name
		return string.lower(ad) < string.lower(bd)
	end)
	return arr
end
...
-- Fly
do
	local Card = makeCard("Fly","Vol fluide (WASD / ZQSD + inclinaison caméra pour vertical)",100)
	local Toggle, setT = makeToggle(Card, false)
	Toggle.MouseButton1Click:Connect(function()
		S.flyOn = not S.flyOn
		setT(S.flyOn)
...
-- ESP
do
	local Card = makeCard("ESP Player","Contours + noms/distance (couleurs rôle/équipe).",130)
	local Toggle, setT = makeToggle(Card, false)
	-- petite ligne d’options
	...
	local function mini(label, default, cb)
		...
		local L = Instance.new("TextLabel") L.BackgroundTransparency=1; L.Font=Enum.Font.Gotham; L.TextSize=14; L.TextXAlignment=Enum.TextXAlignment.Left
		L.TextColor3=Color3.fromRGB(200,200,210); L.Text=label; L.Position=UDim2.fromOffset(0,6); L.Size=UDim2.fromOffset(130,18); L.Parent=Wrap
	...
	-- (tu peux ajouter un limiter distance ici si tu veux)
	Toggle.MouseButton1Click:Connect(function()
		S.espOn = not S.espOn
		setT(S.espOn)
...
do
	...
	refreshBtn.BackgroundColor3 = Color3.fromRGB(50,130,255)
	...
	listPad.PaddingLeft=UDim.new(0,6); listPad.PaddingRight=UDim.new(0,6); listPad.PaddingTop=UDim.new(0,6); listPad.PaddingBottom=UDim.new(0,6)

	local emptyLabel = Instance.new("TextLabel")
	emptyLabel.Size=UDim2.new(1,-12,0,28); emptyLabel.BackgroundTransparency=1
	emptyLabel.Text="Aucun autre joueur."
	emptyLabel.Font=Enum.Font.Gotham; emptyLabel.TextSize=14
	emptyLabel.TextColor3=Color3.fromRGB(200,200,205); emptyLabel.Visible=false
	emptyLabel.Parent = listHolder

	local function setTPEnabled(on)
		tpBtn.Active=on; tpBtn.AutoButtonColor=on
		tpBtn.BackgroundColor3 = on and Color3.fromRGB(60,120,255) or Color3.fromRGB(80,80,90)
	end
		...
		if #others==0 then
			...
			for _,p in ipairs(others) do
				...
				b.MouseButton1Click:Connect(function()
					S.tpSelectedUserId = p.UserId
					S.tpSelectedLabel = (p.DisplayName~="" and p.DisplayName) or p.Name
	...
	-- bouton TP
	_G.__dummy = nil -- keep l'UDim calc stable
	...
	UIS.InputBegan:Connect(function(input, gp)
		...
		if input.KeyCode==Enum.KeyCode.G and S.tpSelectedUserId then tpToPlayerId(S.tpSelectedUserId) end
	end)
end

-- NoClip
do
	local Card = makeCard("NoClip","Traverse les objets (réversible, restaure collisions d’origine).",100)
	local Toggle, setT = makeToggle(Card, false)
	Toggle.MouseButton1Click:Connect(function()
...
-- Infinite Jump
do
	local Card = makeCard("Infinite Jump","Appuie sur sauter autant que tu veux (boost réglable).",150)
	local Toggle, setT = makeToggle(Card, false)
	...
	Toggle.MouseButton1Click:Connect(function()
		...
		if S.ijOn then
			...
			disconnect(S.ijConn); S.ijConn=nil
		end
	end)
end
...
local function clampToScreen(x,y)
	...
	return math.clamp(x,0,math.max(0,vs.X-w)), math.clamp(y,0,math.max(0,vs.Y-h))
end
local dragging, offset
local function beginDrag(io)
	dragging=true
	local p = Main.AbsolutePosition
	offset = Vector2.new(io.Position.X - p.X, io.Position.Y - p.Y)
end
local function updateDrag(io)
	if not dragging then return end
	local x = io.Position.X - offset.X
	local y = io.Position.Y - offset.Y
...
-- ========== FIN ==========
print("[Premium Hub v2] prêt. Ouvre/ferme avec \"$\" (⇧+4), F2, Insert, RightCtrl.")
