--[[
   ...
   - Menu draggable (borné à l'écran) + scroll si nécessaire
   - Sliders: Speed, Jump Boost
   - ESP corrigé: un seul pseudo (anti-doublons) même après respawn/reset
]]
...
local function disconnectAll(list)
	...
	-- remplace table.clear pour compatiblité
...
stroke.Thickness = 1.5; stroke.Transparency = .15; stroke.Color = Color3.fromRGB(255,255,255)
...
Hint.Position = UDim2.new(1,-42,0,10) -- place pour bouton hide
...
local function makeCard(titleText, subtitleText, height)
	...
	ST.Size = UDim2.fromOffset(470,18)
	ST.Parent = Card
...
local function makeSlider(parent, title, minV, maxV, defaultV, yOffset, onChanged)
	...
	T.Size = UDim2.fromOffset(260,18)
	T.Parent = Wrap
	...
	return { set = function(v)
		...
		setFromX(Wrap.AbsoluteSize.X * r)
	end }
end
...
local function speed_enable()
	...
	S.speedCon.prop = S.humanoid:GetPropertyChangedSignal("WalkSpeed"):Connect(function()
		if S.speedOn and S.humanoid.WalkSpeed ~= S.speed then
			S.humanoid.WalkSpeed = S.speed
		end
	end)
end
...
LP.CharacterAdded:Connect(function()
	if S.speedOn then task.wait(.3); speed_enable() end
end)
...
local function fly_enable()
	...
	table.insert(S.flyConns, RS.Stepped:Connect(function()
		if S.flyAlive then hrp.AssemblyAngularVelocity = Vector3.new(0,0,0) end
	end))
	task.spawn(function()
	...
	table.insert(S.flyConns, RS.RenderStepped:Connect(function()
		...
		if yawDir.Magnitude < 1e-3 then yawDir = Vector3.new(0,0,1) else yawDir = yawDir.Unit end
		...
		if rightDir.Magnitude == 0 then rightDir = Vector3.new(-yawDir.Z,0,yawDir.X) else rightDir = rightDir.Unit end

		local fwd, right = 0,0
		if UIS:IsKeyDown(Enum.KeyCode.W) then fwd  = fwd  + 1 end
		if UIS:IsKeyDown(Enum.KeyCode.S) then fwd  = fwd  - 1 end
		...
		if horiz.Magnitude > 0 then horiz = horiz.Unit * SPEED else horiz = Vector3.new(0,0,0) end
...
-- ========= LIGHT =========
local LIGHT_PRESET = {
	GlobalShadows=false, FogEnd=100000, Brightness=2, Ambient=Color3.fromRGB(150,150,150)
}
...
-- Récupère/reparent d'anciens ESP_BBG et supprime les doublons
local function cleanupLegacyBillboards(char)
	local keep = nil
	-- tout Billboard nommé ESP_BBG où qu'il soit -> parent = char
	for _,d in ipairs(char:GetDescendants()) do
		if d:IsA("BillboardGui") and d.Name == "ESP_BBG" and d.Parent ~= char then
			d.Parent = char
		end
	end
	-- s'il y en a plusieurs sous le char -> on en garde un
...
local function ensureESPForChar(p, char)
	...
	-- Billboard unique parenté AU CHARACTER (pas à la Head)
		...
		if p ~= LP then
			...
			if char then
				...
				if tHRP then
					...
					local bb = char:FindFirstChild("ESP_BBG")
					if bb then
						bb.Enabled = not tooFar
						-- garde l'Adornee à jour si la tête pop plus tard
...
local function esp_enable()
	...
	-- nettoie d'abord tout l'historique pour éviter les doublons existants
...
local function esp_disable()
	...
	for _,p in ipairs(Players:GetPlayers()) do
		...
		if char then
			...
			for _,d in ipairs(char:GetDescendants()) do
				if d:IsA("BillboardGui") and d.Name=="ESP_BBG" then d.Enabled=false end
			end
		end
	end
end
...
do
	...
	Row.Position = UDim2.fromOffset(12,56)
	...
	local function mini(label, default, cb)
		local Wrap = Instance.new("Frame")
		Wrap.BackgroundTransparency = 1
		Wrap.Size = UDim2.fromOffset(210,28)
		Wrap.Parent = Row
		...
		L.Size = UDim2.fromOffset(130,18)
		L.Parent = Wrap

		local Btn, setOn = makeToggle(Wrap, default)
...
do
	...
	local function resizeCanvas()
		local total = 0
		for _,ch in ipairs(listHolder:GetChildren()) do
			if ch:IsA("TextButton") then total = total + ch.AbsoluteSize.Y + listLayout.Padding.Offset end
		end
	...
	local function rebuild()
		...
		if #others == 0 then
			...
			for _,p in ipairs(others) do
				...
				b.BackgroundColor3 = Color3.fromRGB(56,56,66)
	...
	tpBtn.MouseButton1Click:Connect(function()
		if S.tpSelectedUserId then tpToPlayerId(S.tpSelectedUserId) end
	end)

	UIS.InputBegan:Connect(function(input, gp)
		if gp then return end
		if input.KeyCode == Enum.KeyCode.G and S.tpSelectedUserId then
			tpToPlayerId(S.tpSelectedUserId)
		end
	end)
end
...
print("[Premium Hub v2] prêt. ESP corrigé (anti-doublons). Ouvre/ferme: \"$\" (⇧+4) • F2 • Insert • RightCtrl.")
